...
\setupTitle
  [title={Optimal design of sequential teams},
   ...
   date={Joint work with:  \crlf
    Ashutosh Nayyar and Demos Teneketzis (UofM); Sekhar Tatikonda (Yale) \blank[medium] Presented at: McGill University, May 20, 2009}]
...
\startitemize
  \head Components of a sequential team
    ...
    \startitemize[unpacked]
      \item A set $N$ of indices of system variables $\{X_n, n \in N\}$ and a partial order \rlap{$\prec$ on $N$}
        \vskip -.5\baselineskip
        \startitemize[packed]
          \item $A \subset N$, denoting the indices of decisions 
          \item $R \subset N$, denoting the indices of reward variables
        \stopitemize
  ...
  \head Design
    ...
    \startitemize
      \item $G_A = \{g_n$, $n \in N\}$, where $g_n$ is a decision rule from
...
\startitemize
  \head The main idea
    ...
    \startalignment[middle]
      If $X_i \in $ data is independent of future rewards given the control
      action and other data at the DM, then it can be ignored
    \stopalignment
...
\SlideTitle A \quotation{decentralized} MDP

\startitemize
  \head Mathematical Model

    \startitemize
      \item Plant: $f_{t-1}(x_t | x_{t-1}, u_{1,t-1}, u_{2,t-1})$
      \item Controller~$i$: $u_{i,t} = g_{i,t}(x_1, \dots, x_t, u_{i,1}, \dots,
        u_{i,t-1})$
      \item Reward: $ρ(r | x_t, u_{1,t}, u_{2,t})$
    \stopitemize
  ...
  \head Graphical Model
    ...
    \vskip -3\baselineskip
\placefigure[here,none]{}
  {\externalfigure[fig-dec-mdp][page=1, width=0.8\textwidth]}
...
\SlideTitle Structural Results for Dec MDP
...
\startitemize
  \head Graphically
    ...
    \vskip -2\baselineskip
    \placefigure[here,none]{}
      {\externalfigure[fig-dec-mdp][page=15, width=0.8\textwidth]}

    \head Mathematically

      \startitemize
        \item Original Controllers:
          $u_{i,t} = g_{i,t}(x_1, \dots, x_t, u_{i,1}, \dots,
          u_{i,t})$

        \item New controllers: 
          $u_{1,t} = g_{1,t}(x_t)$ 
        \nop \hphantom{New controllers:}
          $u_{2,t} = g_{2,t}(x_t, u_{1,t})$ 
...
\startitemize
  ...
  \head Graphical Model
    ...
    \startitemize
      ...
      \item Hierarchy of common observations
    \stopitemize
